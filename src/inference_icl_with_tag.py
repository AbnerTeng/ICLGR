...
class DecoderInferenceWithTag:
    ...
    @staticmethod
    def _extract_query(user_content: str) -> Optional[str]:
        # Last 'Query: ...\nAnswer:' before end of prompt is the real query
        # (earlier ones are demonstration examples).
    ...
    def evaluate_on_test_set(
        ...
        for batch_start in pbar:
            ...
            prompts, true_docids, queries, context_ids_batch = [], [], [], []
            for item in batch:
                ...
                queries.append(self._extract_query(user_content))
                prompts.append(
                    f"<|im_start|>user\n{user_content}<|im_end|>\n<|im_start|>assistant\n"
                )
                if needs_context:
            ...
            for item, true_docid, query, predicted_docids in zip(
                batch, true_docids, queries, predicted_batch
            ):
