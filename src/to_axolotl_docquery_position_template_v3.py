...
Template (Zero-shot Retrieval):
    ## Task
    Query: {query}
    Answer:
    {answer_id}

Template (Zero-shot Indexing):
    ## Task
    Document: {doc_text}
    Answer:
    {answer_id}
...
Formats:
    ...
    4. stage1_retrieval    — No context, query → identifier (retrieval)
    5. stage1_indexing     — No context, document → identifier (indexing)

    Formats 4 & 5 share a combined `stage1` budget (split evenly).
    When `zs_input_path` is set in config, zero-shot data is
    sourced from a separate dataset (e.g. with pseudo queries)
...
def _process_query(args):
    q, n_shot, doc_id_to_doc, eligible_docs, use_retrieval_hn = args
    front = generate_doc_pos_front(
        q, n_docs=n_shot, doc_id_to_doc=doc_id_to_doc,
        eligible_docs=eligible_docs, use_retrieval_hn=use_retrieval_hn,
    )
    ...
    noise = generate_all_noise(
        ...
        doc_id_to_doc=doc_id_to_doc, use_retrieval_hn=use_retrieval_hn,
    )
    return front, back, noise


def _process_noise_only(args):
    q, n_shot, eligible_docs = args
    return generate_all_noise(q, n_docs=n_shot, eligible_docs=eligible_docs)
...
def process_and_save(
    ...
    if zs_noise and zs_docs is not None and zs_queries is not None:
        ...
        buckets["all_noise"] = []
        zs_noise_args = [(q, n_shot, eligible_docs) for q in zs_queries]
        with Pool(processes=n_workers) as pool:
    ...
    if hard_negative:
        ...
        for q in tqdm(hn_queries, desc="Hard negative all_noise"):
            ...
            ex = generate_all_noise_hard(q, n_shot, hn_docs, neg_indices)
            if ex:
    ...
    stage1_retrieval = [generate_zero_shot(q) for q in _zs_queries]
