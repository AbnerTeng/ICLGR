"""
Generate training data with instruction-based template (v4).
...
Changes from v3:
  ...
  - Document format: Each document is wrapped in [DOC_START]...[DOC_END]
    tags with ID and Content fields under a ## Knowledge Base header.

Template (CTX):
    ## Knowledge Base
    [DOC_START] ID: {item1} | Content: {doc1_text} [DOC_END]
    [DOC_START] ID: {item2} | Content: {doc2_text} [DOC_END]
    [DOC_START] ID: {item3} | Content: {doc3_text} [DOC_END]

    ## Retrieval Task
    Query: {query}
    The most relevant ID is:
...
def _build_ctx_prompt(context_docs: List[Dict], query_text: str) -> str:
    doc_lines = [
        f"[DOC_START] ID: {doc['item']} | Content: {doc['text']} [DOC_END]"
